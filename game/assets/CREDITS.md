# Credits & Attribution

This file lists every third-party asset requiring attribution (CC-BY, OFL, etc.)
and is shipped with the game per its license obligations. CC0 assets are listed
in `ASSET_MANIFEST.csv` but do not require attribution here.

## Models
**CC-BY — attribution required:**
- server rack by Jeremy Eyring [CC-BY] via Poly Pizza

Heist props (loot / security / decor):
- ATM by J-Toastie [CC-BY] via Poly Pizza
- Cash Stack by J-Toastie [CC-BY] via Poly Pizza
- Security Camera by J-Toastie [CC-BY] via Poly Pizza
- Security Keypad by Bruno Oliveira [CC-BY] via Poly Pizza
- Biometric Scanner by Wesley Thompson [CC-BY] via Poly Pizza
- Fuse Box by zeoxo [CC-BY] via Poly Pizza
- Display Case by CMHT Oculus [CC-BY] via Poly Pizza
- Gunny Sack by Oliver Herklotz [CC-BY] via Poly Pizza
- diamond by smoj [CC-BY] via Poly Pizza
- Painting by Jonathan Granskog [CC-BY] via Poly Pizza
- Wall Art 01 by Jarlan Perez [CC-BY] via Poly Pizza
- Wall Art 04 by Jarlan Perez [CC-BY] via Poly Pizza
- Wall Art 06 by Jarlan Perez [CC-BY] via Poly Pizza
- Wall painting by jeremy [CC-BY] via Poly Pizza

Heist props (credited as provided by author):
- Safe by CreativeTrio
- Padlock by Quaternius
- Gold Bars by hat_my_guy

CC0 — attribution not required, credited voluntarily (Kenney's license asks for it):
- **Kenney** (kenney.nl) — Modular Buildings, City Kit Commercial, Factory Kit,
  Survival Kit, Furniture Kit. CC0.
- **Quaternius** (quaternius.com) — Modular SciFi MegaKit (Standard), LowPoly
  Furniture, Ultimate Modular Men (characters), LowPoly Guns (weapons), Padlock. CC0.

## Audio (SFX / Music)
CC0 — attribution not required, credited voluntarily:
- **Kenney** (kenney.nl) — Interface Sounds, Impact Sounds. CC0. *(Task 17 wires these as the diegetic
  SFX set — cues approximated from the interface/impact libraries; bespoke sting/klaxon/drill pending.)*
- _music: procedural placeholder beds generated in-code (`AudioManager._make_bed`) for task 17; no real
  music stems yet — see ART-TODO._

## UI / Icons
CC0 — attribution not required, credited voluntarily:
- **Kenney** (kenney.nl) — Game Icons, Game Icons Expansion, UI Pack, UI Pack: RPG
  Expansion. CC0.

## Fonts
CC0:
- **Kenney** — Kenney Future, Kenney Future Narrow. CC0.
- _body/primary font (Google Fonts — Barlow/Inter): none yet_

## Generators / Tools
- _none yet_

> Pipeline rule (GDD §13.5): no asset enters the repo without a row in
> `ASSET_MANIFEST.csv`; CC-BY assets additionally get an entry here.
