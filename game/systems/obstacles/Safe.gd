...
signal minigame_requested(kind: StringName)   ## task 07 mounts the overlay on this; no fake auto-solve here
...
func interact(by: Node) -> void:
	...
	if not String(def.clue_id).is_empty() and Obstacle.actor_has_item(by, def.clue_id):
		...
		minigame_requested.emit(&"safe_dial")   # TODO[07]: mount the dial overlay; solved -> _mark_solved("safe_dial")
