...
Targets: electronic locks, keypads, camera systems (disable or **loop the feed**), alarm panels, vault time-locks, **data loot**. Hacks **take time and demand that you stand there** — a timed hack is a **channelled** interaction, and **moving abandons it**: progress is lost and you start over. (It used to merely *pause* out of range and silently resume when you wandered back — so a hack could finish behind your back with nothing on screen. Abandoning makes the exposure real: a long hack means committing to standing still in the open.) Longer hack = longer exposure. Players who'd rather not be able to fumble it can flip **Options → Controls → "While Interacting"** from *Cancel interaction on movement* to *Lock movement while interacting*, which roots them for the duration and rebinds Interact to "cancel". **Camera loop** is quieter than disabling (no "offline" suspicion) but temporary. **Keypads** use a faster Mastermind-style deduction variant, or a found code.
...
**Drill** (timed, noisy, can jam and need repair, draws guards; upgradeable) · **thermite** (timed burn, very loud) · **C4** (instant breach, max alarm). A breach is the **one timed job you can walk away from** — and that's the point: you set it screaming, it draws guards, and you leave it grinding to go deal with them. It is deliberately **exempt** from the hack's "moving cancels it" rule. What *does* need you back at the door is clearing a **jam** — roughly as close as you had to be to start it. A marquee **Crack** vault is multi-step: *acquire manager's keycard → hack the time-lock → drill/thermite/C4 the door → ferry contents under rising Pursuit.* Where stealth prep and loud execution meet.
